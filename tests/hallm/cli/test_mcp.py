...
def test_serve_defaults(runner: CliRunner) -> None:
...
def test_serve_custom_host(runner: CliRunner) -> None:
    with patch("hallm.mcp.server.run") as mock_run:
...
def test_serve_custom_host_and_port(runner: CliRunner) -> None:
    with patch("hallm.mcp.server.run") as mock_run:
