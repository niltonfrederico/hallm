...
def _paperless_client(handler: Callable[[httpx.Request], httpx.Response]) -> PaperlessClient:
    return mock_http_client(
        PaperlessClient,
        base_url="https://paperless.test",
        handler=handler,
        auth_headers={"Authorization": "Token tok"},
        token="tok",
    )
...
class TestPaperlessClient:
    async def test_upload_document_returns_response_json(self, tmp_path: Path) -> None:
        ...
        async with _paperless_client(handler) as p:
